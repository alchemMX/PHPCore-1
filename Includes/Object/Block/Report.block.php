...
{    
    ...
    public function getLastSolved()
    {
        return $this->db->query('
            ...
            FROM ' . TABLE_REPORTS . '
            ' . $this->join->user('r.report_type_user_id'). '
            LEFT JOIN ' . TABLE_REPORTS_REASONS . ' ON rr.reason_id = (
                SELECT MAX(reason_id)
                FROM ' . TABLE_REPORTS_REASONS . '
                WHERE r.report_id = report_id AND reason_type = 2 
                ORDER BY reason_time DESC
            )
            WHERE report_status = 1
            ORDER BY rr.reason_time DESC
            LIMIT 5
        ', [], ROWS);
    }
    ...
    public function getReasonsCount( int $reportID )
    {
        return (int)$this->db->query('
            ...
            FROM ' . TABLE_REPORTS_REASONS . '
            WHERE reason_type = 1 AND report_id = ?
        ', [$reportID])['count'];
    }
